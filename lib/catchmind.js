...
function submitCatchmind(game, name, strokes, word) {
  ...
  const sub = { id: crypto.randomUUID(), name: trimmed, strokes, word: w, hints, hintsShown: 0, winnerName: null, revealed: false };
...
function revealHint(game) {
  ...
  const total = (sub.hints || []).length;
  if ((sub.hintsShown || 0) < total) sub.hintsShown = (sub.hintsShown || 0) + 1;
  return sub;
}
...
function resetCatchmind(game) {
  ...
  game.currentIndex = -1;
  game.scores = {};
  for (const s of game.submissions) { s.winnerName = null; s.revealed = false; s.hintsShown = 0; }
}
...
function catchmindSnapshot(game) {
  ...
  return {
    ...
    current: cur ? {
      ...
      hints: (cur.hints || []).slice(0, cur.hintsShown || 0), // 공개된 힌트만
      hintsShown: cur.hintsShown || 0,
      hintsTotal: (cur.hints || []).length,
